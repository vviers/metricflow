...
class DataSourceJoinValidator:
    ...
    def _data_source_of_identifier_in_instance_set(
        ...
        assert len(matching_instances) <= 1, f">1 matching identifier instances found: {matching_instances}"

        assert len(matching_instances[0].defined_from) == 1
        return matching_instances[0].defined_from[0].data_source_reference
